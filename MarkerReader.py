...
class MarkerReader():
    ...
    # def _find_control_marker(sample_median, peaks_loc, properties):
    #     l1 = len(sample_median)/4
    #     for 

    def _modify_peaks_width(self, peaks_loc, properties, maximum_width = 30):
        for idx, (peak) in enumerate(peaks_loc):
            if properties['widths'][idx] > maximum_width:
                left_lobe = np.min([np.abs(peak - properties['left_ips'][idx]), maximum_width/2])
                right_lobe = np.min([np.abs(peak - properties['right_ips'][idx]), maximum_width/2])
                properties['left_ips'][idx] = peak - left_lobe
                properties['right_ips'][idx] = peak + right_lobe
        return properties
    ...
    def read_markers(self, sample_cropped):
        ...
        peaks_loc, properties = find_peaks(sample_median, height=0, prominence=20, width=(10, 50), distance=30)
        
        properties = self._modify_peaks_width(peaks_loc, properties)
        ...
        if len(peaks_loc) > 0:
            ...
            #                                         "black")
            control_val = properties['peak_heights'][0] - noise_level
            sample_cropped_marked_boxes = self._draw_box_border(sample_cropped_marked_boxes,
        ...
        test_3_val = None
        if len(peaks_loc) > 1:
            test_1_loc = peaks_loc[1]
            test_1_val = properties['peak_heights'][1] - noise_level
            sample_cropped_marked_boxes = self._draw_box_border(sample_cropped_marked_boxes,
                                                    int(properties['left_ips'][1]) * bin_width + start_margin,
                                                    int(properties['right_ips'][1]) * bin_width + start_margin,
                                                    "red")
        if len(peaks_loc) > 2:
            test_2_loc = peaks_loc[2]
            test_2_val = properties['peak_heights'][2] - noise_level
            sample_cropped_marked_boxes = self._draw_box_border(sample_cropped_marked_boxes,
                                                    int(properties['left_ips'][2]) * bin_width + start_margin,
                                                    int(properties['right_ips'][2]) * bin_width + start_margin,
                                                    "orange")
        ...
        if len(peaks_loc) > 3:
            test_3_loc = peaks_loc[3]
            test_3_val = properties['peak_heights'][3] - noise_level
            sample_cropped_marked_boxes = self._draw_box_border(sample_cropped_marked_boxes,
                                                    int(properties['left_ips'][3]) * bin_width + start_margin,
                                                    int(properties['right_ips'][3]) * bin_width + start_margin,
                                                    "cyan")

        if len(peaks_loc) > 4:
            for i in np.arange(4,len(peaks_loc), step=1):
                sample_cropped_marked_boxes = self._draw_box_border(sample_cropped_marked_boxes,
        ...
        if test_2_val is not None:
            ...
            test_2_val = 0
            p_value_2 = 1
        if test_3_val is not None:
            print(test_3_val/control_val)
            z_score = test_3_val/noise_std    
            p_value_3 = stats.norm.sf(abs(z_score))
        else:
            test_3_val = 0
            p_value_3 = 1

        return sample_cropped_marked_boxes, (control_val, test_1_val, test_2_val, test_3_val), (0, p_value_1, p_value_2, p_value_3)
